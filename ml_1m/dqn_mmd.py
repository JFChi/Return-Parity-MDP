...
from env import Env
from models import MLP, MMDStatistic
...
class DQN():
    def __init__(self, config, args):
        ...
        # add mmd-specific parameters
        self.mmd = MMDStatistic(args.kernel_bandwidths, kernel_name="gaussian")
        self.mmd_update_interval = args.mmd_update_interval
        self.mmd_batch_size = args.mmd_batch_size
        # mmd_optim_params = [self.g0_feat_ext.parameters(), self.g1_feat_ext.parameters()]
        self.mmd_optim_g0 = torch.optim.Adam(self.g0_feat_ext.parameters(), lr=self.learning_rate)
        self.mmd_optim_g1 = torch.optim.Adam(self.g1_feat_ext.parameters(), lr=self.learning_rate)

        self.logger.info(f"mmd_update_interval: {self.mmd_update_interval}")
    ...
    def run(self):
        ...
        for i in range(0, self.max_training_step):
            ...
            if i % self.log_step == 0:
                ...
                ## logging: used for mmd alignment
    ...
    def update(self):
        ...
        # update mmd loss after updating other components
        # in each mmd update step size (in update function), first do the sampling by groups (under no grad env), evaluate their return,
        # after collect the states, pass them to the feat ext and train
        if self.update_steps % self.mmd_update_interval == 0:
            ...
            state_g0_feats_for_alignment = self.g0_feat_ext(states_g0_for_alignment)
            state_g1_feats_for_alignment = self.g1_feat_ext(states_g1_for_alignment)

            # mmd loss might be too large, might need to decrease the the learning rate
            mmd_loss = self.mmd(state_g0_feats_for_alignment, state_g1_feats_for_alignment)

            if self.curr_test_g0_avg_return > self.curr_test_g1_avg_return:
                # g0 is the privileged group now
                update_params(self.mmd_optim_g0, mmd_loss)
            else:
                # g1 is the privileged group now
                update_params(self.mmd_optim_g1, mmd_loss)

            if self.update_steps % 20 == 0:
                self.logger.info(f"In update step {self.update_steps}, mmd loss: {mmd_loss.item():.4f}")            
                self.epoch_logger.store(
                    model_update_steps=self.update_steps, # model_update_steps
    ...
    def sample_for_alignment(self, group_idx):
        ...
        group_sample_batch = self.mmd_batch_size
        userID_train_set = self.g0_userID_train_set if group_idx == "g0" else self.g1_userID_train_set
        rnn_model = self.rnn_g0 if group_idx == "g0" else self.rnn_g1
        feat_ext = self.g0_feat_ext if group_idx == "g0" else self.g1_feat_ext
