...
class DQN():
    def __init__(self, config, args):
        ...
        # load pretrained rnn from state
        # if not args.no_pretrain_rnn:
        #     self.logger.info(f"Loading pretrained rnn model from {self.rnn_file_path}")
        #     self.rnn_g0.load_state_dict(torch.load(self.rnn_file_path))
        #     self.rnn_g1.load_state_dict(torch.load(self.rnn_file_path))
        # else:
        #     self.logger.info(f"rnn init randomly")
        # self.logger.info(f"Only one group (g0) pretrain")
        ...
        # define optim
        self.qnet_optim = self.optim = torch.optim.Adam(self.qnet.parameters(), lr=self.learning_rate)
    ...
    def update(self):
        ...
        states = np.concatenate((states_g0, states_g1), axis=0)
        actions = np.concatenate((actions_g0, actions_g1), axis=0)
        rewards = np.concatenate((rewards_g0, rewards_g1), axis=0)
        next_states = np.concatenate((next_states_g0, next_states_g1), axis=0)
        ...
        q_value = self.qnet(states).gather(dim=1, index=actions.unsqueeze(1))


        # get target q value
        with torch.no_grad():
            max_q, _ = torch.max(self.target_qnet(next_states), dim=1)
    ...
    def sample(self, group_idx):
        ...
        while True:
            ...
            argmax_action = torch.argmax(self.qnet(tmp_state) - sampled_action_mask, dim=-1)
